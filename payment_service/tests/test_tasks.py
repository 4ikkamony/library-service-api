...
class ExpirePaymentsTestCase(TestCase):
    def setUp(self):
        ...
        self.patcher_expired_sessions = patch(
            "payment_service.tasks.expired_sessions", new=self.mock_expired_sessions
        )
    ...
    def test_expire_payments_database_error(self):
        self.mock_expired_sessions.side_effect = Exception("Database connection failed")
...
class NotifyNewPaymentTestCase(TestCase):
    def setUp(self):
        ...
        self.patcher_send_telegram = patch(
            "payment_service.tasks.send_telegram_message", new=self.mock_send_telegram
        )
    ...
    def test_notify_new_payment_not_found(self):
        ...
        self.assertEqual(
            str(context.exception), f"Payment with ID {non_existent_id} not found"
        )
...
class NotifySuccessfulPaymentTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.book = Book.objects.create(
            title="Test Book", author="Author", inventory=30, daily_fee=5.00
        )
        self.borrowing = Borrowing.objects.create(
            user=self.user,
            book=self.book,
    ...
    @patch("payment_service.tasks.send_telegram_message")
    def test_notify_successful_payment_status_not_paid(self, mock_send_telegram):
        mock_send_telegram.return_value = True
        result = notify_successful_payment(self.payment_pending.id)
        mock_send_telegram.assert_not_called()

    @patch("payment_service.tasks.send_telegram_message")
    def test_notify_successful_payment_payment_not_found(self, mock_send_telegram):
        mock_send_telegram.return_value = True
